...
Delta.prototype.slice = function (start, end) {
  ...
  while (index < end && iter.hasNext()) {
    var nextOp = iter.next(end - start);
    if (start <= index) {
      delta.push(nextOp);
    }
    ...
    while (length > 0) {
      ...
      switch (component[0]) {
        case diff.INSERT:
          ...
          this.push(otherIter.next(opLength));
          break;
        case diff.DELETE:
          opLength = Math.min(length, thisIter.peekLength());
          thisIter.next(opLength);
          this['delete'](opLength);
          break;
        case diff.EQUAL:
          ...
          if (thisOp.insert === otherOp.insert) {
            this.retain(opLength, op.attributes.diff(thisOp.attributes, otherOp.attributes));
          } else {
            this.push(otherOp)['delete'](opLength);
          }
          break;
      }
      length -= opLength;
