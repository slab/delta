  ...
  while (thisIter.hasNext() || otherIter.hasNext()) {
    if (otherIter.peekType() === 'insert') {
      ...
      if (is.number(otherOp.retain)) {
        ...
        // Preserve null when composing with a retain and remove it for inserts
        var formats = op.format.compose(thisOp.formats, otherOp.formats, is.number(thisOp.retain));
        if (formats) {
          newOp.formats = formats;
        } else if (thisOp.formats) {
          // Edge case of retaining and empty embed
          newOp.formats = {};
        }
        this._push(newOp);
  ...
  while (thisIter.hasNext() || otherIter.hasNext()) {
    if (thisIter.peekType() === 'insert' && (priority || otherIter.peekType() !== 'insert')) {
      ...
      if (thisOp.delete) {
        ...
        // We are both retains
        this.retain(length, op.format.transform(thisOp.formats, otherOp.formats, priority));
      }
    }
  }
  return this._chop();
