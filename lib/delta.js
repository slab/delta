...
Delta.prototype.diff = function (other) {
  ...
  diffResult.forEach(function (component) {
    ...
    switch (component[0]) {
      case diff.INSERT:
        while (length > 0) {
          var opLength = Math.min(otherIter.peekLength(), length);
          var nextOp = otherIter.next(opLength);
          delta.push(nextOp);
          length -= opLength;
        }
        break;
      ...
      case diff.EQUAL:
        while (length > 0) {
          var opLength = Math.min(thisIter.peekLength(), otherIter.peekLength(), length);
