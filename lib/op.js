...
var lib = {
  composeFormats: function(a, b) {
    ...
    var newFormat = {};
    for (var key in a) {
      // In a and not specified to be removed in b
...
Iterator.prototype.peekLength = function() {
  if (this.ops[index]) {
    ...
    return lib.length(this.ops[index]) - this.offset;
  } else {
    return Infinity;
  }
};
...
Iterator.prototype.peekType = function() {
  if (this.ops[index]) {
    if (is.number(this.ops[index].delete)) {
      return 'delete';
    } else if (is.number(this.ops[index].retain)) {
      return 'retain';
    } else {
      return 'insert';
    }
